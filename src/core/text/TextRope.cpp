#include "core/text/TextRope.h"

#include <algorithm>

namespace diffy {

void TextRope::clear() {
  chunks_.clear();
  size_ = 0;
}

TextRange TextRope::append(std::string_view text) {
  const TextRange range{size_, text.size()};
  if (!text.empty()) {
    chunks_.push_back(Chunk{size_, std::string(text)});
    size_ += text.size();
  }
  return range;
}

std::string TextRope::slice(const TextRange& range) const {
  if (range.length == 0 || chunks_.empty()) {
    return {};
  }

  const size_t end = range.start + range.length;
  std::string result;
  result.reserve(range.length);

  for (const Chunk& chunk : chunks_) {
    const size_t chunkStart = chunk.start;
    const size_t chunkEnd = chunk.start + chunk.text.size();
    if (chunkEnd <= range.start) {
      continue;
    }
    if (chunkStart >= end) {
      break;
    }

    const size_t sliceStart = std::max(range.start, chunkStart);
    const size_t sliceEnd = std::min(end, chunkEnd);
    const size_t localStart = sliceStart - chunkStart;
    const size_t localLength = sliceEnd - sliceStart;
    result.append(chunk.text.substr(localStart, localLength));
  }

  return result;
}

size_t TextRope::size() const {
  return size_;
}

}  // namespace diffy
